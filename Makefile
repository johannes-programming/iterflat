...
test:
	conda run -n base python make/env.py test_iterflat --python=3.11 --recreate >/dev/null;
	conda run -n test_iterflat pip install -e . >/dev/null;
	conda run -n test_iterflat python -c 'import iterflat.tests; iterflat.tests.test();'
	conda run -n test_iterflat pip install mypy >/dev/null;
	conda run -n test_iterflat python -m mypy --strict .;
	conda run -n test_iterflat python -m mypy --strict -p iterflat;
