.ONESHELL:
.PHONY: all beautiful black build clean commit isort jacobus pypi test toml_sorted_pyproject upload version works
...
test:
	...
	conda run -n test311 python -c 'import iterflat.tests; iterflat.tests.test()';
	conda run -n test311 pip install mypy;
	conda run -n test311 python -m mypy -p iterflat;
